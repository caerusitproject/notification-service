...
public class NotificationOrchestratorService {
        ...
        .forEach(
            channelName -> {
              try {
                ...
                log.error(
                    "Error while processing channel {} for user {}: {}",
                    channelName,
                    event.userId(),
                    e.getMessage(),
                    e);
  ...
  private NotificationService resolveService(Channel channel) {
    return switch (channel) {
      ...
      case IN_APP ->
          throw new UnsupportedOperationException("IN_APP notifications not yet supported");
    };
  }

  private void persistAndSend(
      NotificationEvent event, Channel channel, NotificationService notificationService) {
    ...
    try {
      ...
      if (channel == Channel.EMAIL) {
        notification.setSubject(
            ((EmailNotificationService) notificationService).resolveSubject(event.eventType()));
      }
