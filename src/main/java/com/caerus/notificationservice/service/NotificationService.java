...
public class NotificationService {
	...
	public Notification send(SendRequest req) {
		...
		try {
			prefOpt = preferenceRepository.findByEmail(req.getEmail());
		...
		Map<String, Object> data =  prepareMessage(req);
	...
	public Notification  processMessage(String message) {
		...
		try {
			...
			sendRequest.setFullName(notification.getFullName());
			sendRequest.setEmail(notification.getEmail());
			sendRequest.setPhoneNumber(notification.getPhoneNumber());
			
			emailTemplate = notificationtemplateService.getTemplateByEventType(notification.getEventType(), sendRequest.getChannel());
			sendRequest.setSubject(emailTemplate.getSubject());
			sendRequest.setContent(emailTemplate.getContent());
			sendRequest.setData(prepareMessage(sendRequest));
			sendRequest.setTemplateName(emailTemplate.getTemplateName());
			sendRequest.setTemplateType(emailTemplate.getType());
			sendRequest.setData(prepareMessage(sendRequest));
			
			
			prefOpt = preferenceRepository.findByEmail(notification.getEmail());
	...
	private HashMap<String, Object> prepareMessage(SendRequest req) {
		HashMap<String, Object> message = new HashMap<>();
		message.put("userId", req.getUserId());
		message.put("name", req.getFullName()!=null?req.getFullName():"User");
		message.put("email", req.getEmail());
		message.put("phone", req.getPhoneNumber());
		message.put("username", req.getEmail());
		return message;
	}
}
