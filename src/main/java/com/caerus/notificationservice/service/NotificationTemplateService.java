...
public class NotificationTemplateService {
  ...
  public NotificationTemplate saveTemplate(NotificationTemplate template) {
    if (templateRepository.existsByTemplateName(template.getTemplateName())) {
      throw new IllegalArgumentException(
          "Template with fullName " + template.getTemplateName() + " already exists.");
    }

    NotificationTemplate nTemplate =
  ...
  public NotificationTemplate getTemplate(String templateName, String templateType) {
    ...
    return templateRepository
        .findByTemplateNameAndType(templateName, templateType)
        .orElseThrow(
            () ->
                new IllegalArgumentException(
                    "Template not found with fullName: "
                        + templateName
                        + " and type: "
                        + templateType));
  }
  ...
  public NotificationTemplate getTemplateByEventType(String eventtype, Channel channel) {
    ...
    return templateRepository.findAll().stream()
        .filter(
            t ->
                t.getEventType().equalsIgnoreCase(eventtype)
                    && t.getType().equalsIgnoreCase(channel.name()))
        .findFirst()
        .orElseThrow(
            () ->
                new IllegalArgumentException(
                    "Template not found with event type: "
                        + eventtype
                        + " and channel: "
                        + channel));
  }
  ...
  // TODO
  public NotificationTemplate uploadTemplateFile(
      String templateName,
      String type,
